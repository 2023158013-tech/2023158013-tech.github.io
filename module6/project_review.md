...
* **AS-IS**: 신한은행·카카오뱅크 등 은행마다 CSV 컬럼 구성이 달라, 추상화된 컬럼을 가진 VitaS DB에 저장하는데에 문제가 있었습니다.
...
> ⚠️ **남는 한계**: 잔액은 중복 검증을 위해서만 쓰이고 조회는 불가능하므로 직접 등록(수동 입력)은 잔액을 입력받지 않았습니다. 직접 등록끼리는 잔액을 제외한 나머지가 전부 같으면 중복으로 취급됩니다. 파일 업로드보다 자유롭게 수정 가능한 직접 등록의 특성을 고려해, 감수할 만한 트레이드오프로 판단했습니다.
...
> 💡 **CTE란** — `WITH 이름 AS (...)`로 임시 결과에 이름을 붙여두고, 본문 쿼리에서 그 이름을 테이블처럼 재사용하는 문법입니다. 정산 매칭 추천 쿼리에서 프로젝트의 정산 블록 목록을 한 번 조인해 이름 붙여두고, 최대회차번호·추천총액을 그 이름에서 여러 번 꺼내 쓰는 식으로 조인을 3번에서 1번으로 줄였습니다.
...
재무 도메인은 CSV 일괄 업로드는 업로드 특성상 시간이 걸리는 작업임을 인지하지만 조회가 느리면 사용자 경험에서 치명적이라 생각했습니다. 이 비대칭을 근거로 인덱스 추가를 결정했습니다.

---

## 3. 트러블슈팅 (Troubleshooting)

### 3-1. 이미지 — S3 프라이빗 버킷과 삭제 연쇄 검증
...
빠른 속도로 개발해서 프로젝트 기획의 안정성을 잡는데 기여했습니다.

* **블록 생성/삭제/조회 책임**: 같은 생명주기이지만 각 팀원들이 도메인을 분리해야 했기에 어떻게 생성하고 조회할지가 문제였습니다. 간단한 텍스트, 체크리스트를 선두 개발하여 프로젝트 도메인 담당자가 기획 안정성을 잡는데 기여했습니다.
* **활동 로그 수집 단위**: 전반적인 블록 관련 로그를 수집해야했던 로그 도메인 담당자에게 빠른 개발을 통해 기능이 제대로 동작하는지 검증하며 연동하는 데에 기여했습니다.
...
솔직히 어느 순간부터는 AI(Claude Code)가 만들고 AI(CodeRabbit)가 검증하는 루프에 제가 얼마나 기여하고 있는 건지 의문이 들 때도 있었습니다. 하지만 그 사이에서 제가 할 수 있는 역할은 **"왜 이 방식이어야 하는지"를 판단하는 것**이었습니다 — 세금계산서와 입출금의 동시성 처리를 다르게 설계한 이유, 재무 도메인 구조를 되돌리지 않기로 한 판단, 거래처 관계를 다시 설계한다면 어떤 방향으로 갈지에 대한 트레이드오프와 기능이 기획대로 동작하는지 검증하는 등의 결정들은 결국 도메인을 이해하고 있는 사람이 내려야 하는 몫이었습니다.
